...
model {
  ...
  b2 ~ normal(0,10);
  b3 ~ normal(0,10);
  y ~ lognormal(b0 + b1*x1 + b2*x2 + b3*x3, sigma);
}
...
generated quantities {
  ...
  vector[N] log_lik;
  {
    array[N] real y_rep = lognormal_rng(b0 + b1*x1 + b2*x2 + b3*x3, sigma);
    meanlog_gt = mean(y_rep) > mean(y);
    sdlog_gt = sd(y_rep) > sd(y);
    for (i in 1:N) log_lik[i] = lognormal_lpdf(y[i] | b0 + b1*x1[i] + b2*x2[i] + b3*x3[i], sigma);
  }
}
