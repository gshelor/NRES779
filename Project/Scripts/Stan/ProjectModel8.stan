...
data {
  ...
  vector[N] x2; // temp max
  vector[N] x3; // temp min
  vector[N] x4; // daily precip
}

// The parameters accepted by the model. Our model
// accepts parameters b0, b1, b2, b3, b4, b5, b6, and 'sigma'.
parameters {
  real b0; // intercept
  real b1; // elevation
  real b2; // temp max
  real b3; // temp min
  real b4; // daily precip
  real<lower=0> sigma;
}
...
model {
  ...
  b3 ~ normal(0,10);
  b4 ~ normal(0,10);
  y ~ lognormal(b0 + b1*x1 + b2*x2 + b3*x3^3 + b4*x4, sigma);
}
...
generated quantities {
  ...
  vector[N] log_lik;
  {
    array[N] real y_rep = lognormal_rng(b0 + b1*x1 + b2*x2 + b3*x3^3 + b4*x4, sigma);
    meanlog_gt = mean(y_rep) > mean(y);
    sdlog_gt = sd(y_rep) > sd(y);
    for (i in 1:N) log_lik[i] = lognormal_lpdf(y[i] | b0 + b1*x1[i] + b2*x2[i] + b3*x3[i] + b4*x4[i], sigma);
  }
}
