...
plot_pop = coda.samples(jm_pop, variable.names = c("K", "r", "sigma", "tau"),
                      n.iter = n.iter, n.thin = 1)
zj_pop = jags.samples(jm_pop, variable.names = c("K", "r", "sigma", "tau", "mu"), n.iter = n.iter, n.thin = 1)
...
mu_summary = summary(zj_pop$mu, quantile, c(.025, .5, .975))$stat
par(mfrow = c(1,1), mar = c(4,4,4,4))
plot(Logistic$PopulationSize, mu_summary[2,], type = 'l', xlab = "N (Population)", ylab = "Growth Rate", main = "Median Growth Rate as a Function of Population, with 95% Credible Intervals")
lines(Logistic$PopulationSize, mu_summary[1,], lty= 2)
lines(Logistic$PopulationSize, mu_summary[3,], lty = 2)
...
zm_liz = coda.samples(jm_liz, variable.names = c("b0", "b1", "p"),
                      n.iter = n.iter, n.thin = 1)
plot_liz = coda.samples(jm_liz, variable.names = c("b0", "b1"), n.iter = n.iter, n.thin = 1)
zj_liz = jags.samples(jm_liz, variable.names = c("b0", "b1", "p"), n.iter = n.iter, n.thin = 1)
## estimating posterior dist
